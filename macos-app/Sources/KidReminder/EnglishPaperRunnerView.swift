...
struct EnglishPaperRunnerView: View {
    ...
    /// The step just past the furthest one containing any answered item — so
    /// resuming an interrupted paper picks up where the kid left off instead of
    /// restarting from question 1. Deliberately "furthest reached" rather than
    /// "first unanswered": progression through a paper is strictly sequential
    /// (advance()/skipCurrent() only ever move forward), so a step skipped via
    /// the skip button stays permanently unanswered — "first unanswered" would
    /// send a kid who skipped Q5, answered Q6-40, then force-quit right back to
    /// the already-passed Q5 instead of continuing from Q41. "Furthest reached"
    /// naturally handles both the fresh-session case (no step answered yet ->
    /// 0) and the all-answered case (clamped to the last valid index) without a
    /// separate fallback branch.
    private func resumeStepIndex() -> Int {
        let allSteps = steps
        guard let lastAnswered = allSteps.lastIndex(where: { $0.contains { $0.answered } }) else {
            return 0
        }
        return min(lastAnswered + 1, allSteps.count - 1)
    }
